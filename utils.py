import numpy as np
from sklearn.metrics import roc_auc_score, jaccard_score, precision_score, recall_score
...
def acc_on_batch(labels, pred):
    '''Computes the mean accuracy over a batch during training'''
    accuracies = []
    for i in range(pred.shape[0]):
        predicted_labels = (pred[i][0].cpu().detach().numpy() >= 0.5).astype(int)  # 预测标签，大于等于0.5为正类，否则为负类
        true_labels = labels[i].cpu().detach().numpy()  # 真实标签
...
def iou_on_batch(masks, pred):
    ...
    for i in range(pred.shape[0]):
        ...
        # Compute Precision
        precision = precision_score(mask_tmp.reshape(-1), pred_tmp.reshape(-1))
        precisions.append(precision)

        # Compute Recall
        recall = recall_score(mask_tmp.reshape(-1), pred_tmp.reshape(-1))
        recalls.append(recall)
    return np.mean(ious), np.mean(precisions), np.mean(recalls)


def precision_on_batch(masks, pred):
    '''Computes the mean Precision over a batch during training'''
    precisions = []
...
def recall_on_batch(masks, pred):
    '''Computes the mean Recall over a batch during training'''
    recalls = []
...
def f1_score_on_batch(masks, pred):
    '''Computes the mean F1 score over a batch during training'''
    f1_scores = []
